...
-- Quest Objective
BANETO_DefineQuestTargetId(62984) -- Stunned Whitepetal Carp
...
-- Next Quest in Chain
BANETO_SetNextLocalQuestProfile([[Golden_Lotus_Whitepetal_Lake_05_Stick_in_the_Mud]])
...
function _G.BANETO_ExecuteCustomQuestPulse()
    ...
    if not BANETO_HasQuest(30291) then
        BANETO_Print("Quest 'Stunning Display' (30291) not available, skipping...")
        BANETO_LoadQuestProfile([[Golden_Lotus_Whitepetal_Lake_05_Stick_in_the_Mud]])
        return
    end

    -- Check if quest is completed
    if BANETO_HasQuestCompleted(30291) then
        BANETO_Print("Quest 'Stunning Display' (30291) completed!")
        BANETO_LoadQuestProfile([[Golden_Lotus_Whitepetal_Lake_05_Stick_in_the_Mud]])
        return
    end
